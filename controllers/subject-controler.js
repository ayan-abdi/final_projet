...
const subjectController = {
  ...
  add: async (req, res) => {
    ...
    try {
      ...
      const subjectAdded = await db.sequelize.create(data, { transaction });

      // Ajout d'un element categories via l'id(if existed id)
      await subjectAdded.addCategories(data.Categories, { transaction });

      await transaction.commit();

      res.json(new SuccessObjectRes(subjectAdded)); // <=> Si on met pas le status d
    } catch (error) {
